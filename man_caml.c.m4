...
CAMLprim cudd_caml_man_Cudd_Init_bytecode(value* argv, int argn)
{
  return cudd_caml_man_make(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5]);
}

FUN_1(man,Cudd_DebugCheck,man_t,bool,unit,
	[[
	  Cudd_ErrorType error = Cudd_DebugCheck(man);
	  fflush(man->err);
	  if (error == CUDD_OUT_OF_MEM)
...
CAMLprim cudd_caml_Cudd_ReorderingStatus(value vman)
{
  ...
  if (b){
    vr = alloc_small(1,0);
    Field(vr) = cudd_caml_reorder_t_c2ml(xr);
  } else {
    vr = Val_int(0);
  }
  return vr;
...
FUN_2(man,Cuddaux_SetVarMap,man_t,intarray__t,unit,
      [[
	bool ok = Cuddaux_SetVarMap(man,x1.array,x1.size);
	if (ok==false) caml_failwith(\"Cudd.Man.set_varmap failed\");
	free(x1.array);
	]])
CAMLprim cudd_caml_man_get_params(value vman)
{
  CAMLparam1(vman);
  CAMLlocal1(vres);
  man_t man = cudd_caml_man_t_ml2c(vman);
  ...
  Store_field(vres,8,Val_int(Cudd_ReadDiftMaxVar(man)));
  ...
  Store_field(vres,16,caml_copy_double(Cudd_ReadMaxGrowthAlt(man)));
  Store_field(vres,17,Val_int(Cudd_ReadReorderingCycle(man)));
  Store_field(vres,18,Val_int(Cudd_ReadNextReordering(man)));
  CAMLreturn vres;
}
CAMLprim cudd_caml_man_set_params(value vman,value v)
{
  CAMLparam2(vman,v);
  man_t man = cudd_caml_man_t_ml2c(vman);
  ...
  Cudd_SetDiftMaxVar(man,Int_val(Field(v,8)));
  ...
  Cudd_ReadMaxGrowth(man,Double_val(Field(v,15)));
  Cudd_ReadMaxGrowthAlt(man,Double_val(Field(v,16)));
  Cudd_SetReorderingCycle(man,Int_val(Field(v,17)));
  Cudd_SetNextReordering(man,Int_val(Field(v,18)));
  CAMLreturn Val_unit;
}
CAMLprim cudd_caml_man_stats(value vman)
{
  CAMLparam1(vman);
  CAMLlocal1(vres);
  man_t man = cudd_caml_man_t_ml2c(vman);
  vres = caml_alloc(21,0);
  Store_field(vres,0,caml_copy_double(CuddReadCacheHits(man)));
  Store_field(vres,1,caml_copy_double(CuddReadCacheLookUps(man)));
  Store_field(vres,2,Val_int(Cudd_ReadCacheSlots(man)));
  Store_field(vres,3,caml_copy_double(CuddReadCacheUsedSlots(man)));
  ...
  Store_field(vres,9,Val_int(Cudd_ReadMaxCache(man)));
  Store_field(vres,10,Val_int(Cudd_ReadMinDead(man)));
  Store_field(vres,11,Val_int(Cudd_ReadNodeCount(man)));
  Store_field(vres,12,Val_int(Cudd_ReadPeakNodeCount(man)));
  Store_field(vres,13,Val_int(Cudd_ReadPeakLiveNodeCount(man)));
  Store_field(vres,14,Val_int(Cudd_ReadReorderingTime(man)));
  Store_field(vres,15,Val_int(Cudd_ReadReorderings(man)));
  Store_field(vres,16,Val_int(Cudd_ReadSize(man)));
  Store_field(vres,17,Val_int(Cudd_ReadZddSize(man)));
  Store_field(vres,18,Val_int(Cudd_ReadSlots(man)));
  Store_field(vres,19,caml_copy_double(Cudd_ReadUsedSlots(man)));
